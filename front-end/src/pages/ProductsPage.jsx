    ...
    <>
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-row">
            <div className="field">
              ...
              <input
                id="p-name" type="text" placeholder="Teclado Mecânico"
                value={formData.name}
            ...
            <div className="field">
              ...
              <input
                id="p-price" type="number" step="0.01" min="0" placeholder="249.90"
      ...
      <div className="toolbar">
        <div className="toolbar-left">
          <span className="toolbar-count">{filtered.length} item{filtered.length !== 1 ? 's' : ''}</span>
        </div>
        <div className="toolbar-left">
          ...
          <button className="btn-refresh" onClick={fetchProductsList} title="Recarregar">
            <RefreshIcon />
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-well">
          ...
          <p>{search ? 'Tente outro termo.' : 'Adicione o primeiro produto acima.'}</p>
        </div>
      ) : (
        <div className="catalog-grid">
          {filtered.map((product) => (
            <div className="catalog-item" key={product.id || product.name}>
              ...
              <div className="catalog-item-price">{formatBRL(product.price)}</div>
              <div className="catalog-item-id">{product.id || '—'}</div>
              <div className="catalog-item-actions">
                <button onClick={() => startEditing(product)}>
                  <EditIcon /> Preço
                </button>
                <button className="danger" onClick={() => handleDelete(product.name)} title="Excluir">
                  <TrashIcon />
                </button>
              </div>
            </div>
          ...
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveEdit}>
              ...
              <div className="dialog-footer">
                ...
                <button type="submit" className="btn btn-accent">Atualizar</button>
              </div>
            </form>
          </div>
        </div>
